...
task("deploy", "Deploy the oracle contract")
  ...
  .setAction(async ({ roflAppID }, hre) => {
    ...
    const oracle = await hre.ethers.deployContract(
      "Oracle",
      [rawAppID, threshold],
      {}
    );
    ...
    console.log(
      `Oracle for ROFL app ${roflAppID} deployed to ${oracle.target}`
    );
  });
...
task("oracle-query", "Queries the oracle contract")
  ...
  .setAction(async ({ contractAddress }, { ethers }) => {
    ...
    // TODO: Move below to a ROFL helper library (@oasisprotocol/rofl).
    const roflAppID = bech32.encode(
      "rofl",
      bech32.toWords(ethers.getBytes(rawRoflAppID))
    );
